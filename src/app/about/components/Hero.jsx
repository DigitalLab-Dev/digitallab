...
const Hero = () => {
  ...
  const orangeWords = useMemo(() => "Collaborating with brands stuck in".split(' '), []);
  const whiteWords = useMemo(() => "the noise to build their unique identity".split(' '), []);
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center relative z-10 w-full gap-8 lg:gap-0">
        ...
        <div className="w-full lg:w-2/3 relative">
          ...
          {/* Animated Underline */}
          <div
            className={`h-1 bg-gradient-to-r from-orange-500 via-orange-400 to-white mt-6 sm:mt-8 origin-left transition-all duration-1500 ease-out ${textVisible ? 'scale-x-100' : 'scale-x-0'
              }`}
        ...
        <div className="relative lg:absolute lg:right-10 xl:right-20 lg:-top-30 w-full max-w-md lg:max-w-lg xl:max-w-xl">
          ...
          >
            {/* Glowing Ring Effect */}
            <div
              className={`absolute inset-0 rounded-full transition-all duration-3000 ${textVisible ? 'animate-spin' : ''
                }`}
            ...
            {/* Pulsing Background */}
            <div
              className={`absolute inset-0 bg-gradient-to-r from-orange-500/10 via-transparent to-white/5 rounded-full blur-xl transition-all duration-1000 ${textVisible ? 'animate-pulse scale-110' : 'scale-100'
                }`}
              aria-hidden="true"
            />

...
import { TrendingUp, Target, DollarSign, Play, ArrowRight, BarChart3, Users, Zap, Star } from 'lucide-react';
...
const AdsHero = () => {
  ...
  const floatingElements = [
    ...
    { id: 2, icon: TrendingUp, delay: 1.5, x: '70%', y: '20%', size: 'w-10 h-10' },
    { id: 3, icon: Target, delay: 3, x: '20%', y: '60%', size: 'w-8 h-8' },
    { id: 4, icon: Zap, delay: 4.5, x: '80%', y: '70%', size: 'w-14 h-14' },
    { id: 5, icon: DollarSign, delay: 2, x: '85%', y: '45%', size: 'w-10 h-10' },
    { id: 6, icon: Users, delay: 3.5, x: '25%', y: '80%', size: 'w-12 h-12' }
  ];

  const adMockups = [
    { platform: 'Google Ads', color: 'from-blue-500 to-blue-700', stats: '+250% ROI', delay: 1 },
    { platform: 'Facebook', color: 'from-blue-600 to-purple-700', stats: '89% CTR', delay: 2 },
    { platform: 'TikTok', color: 'from-pink-500 to-red-600', stats: '2.4M Reach', delay: 3 }
  ];
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {floatingElements.map((element) => (
          <div
            key={element.id}
            className={`absolute right-0 w-1/2 ${element.size} transform transition-all duration-1000 ${
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center min-h-screen py-20">
          ...
          <div className="space-y-8 lg:space-y-10">
            ...
            {/* Subheadline */}
            <p className={`text-lg sm:text-xl lg:text-xl text-gray-300 leading-relaxed max-w-2xl transform transition-all duration-1000 delay-400 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}>
              Maximize your <span className="text-orange-400 font-semibold">ROI</span> with precision 
              <span className="text-orange-400 font-semibold"> targeting</span> that delivers 
              <span className="text-orange-400 font-semibold"> measurable results</span>. 
              Transform your ad spend into profitable growth.
            </p>

            {/* Stats Row */}
            <div className={`grid grid-cols-3 gap-4 lg:gap-6 transform transition-all duration-1000 delay-600 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}>
              {[
                ...
                <div key={index} className="text-center group">
                  ...
                  <div className="text-xl lg:text-2xl xl:text-3xl font-bold text-white">{stat.value}</div>
            ...
            {/* CTA Section */}
            <div className={`space-y-4 transform transition-all duration-1000 delay-800 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}>
                ...
                {adMockups.map((ad, index) => (
                  <div
                    ...
                    className={`transform transition-all duration-1000 hover:scale-105 ${
                      index % 2 === 0 ? 'translate-x-0' : 'translate-x-8 lg:translate-x-12'
                    }`}
                    style={{
                  ...
                  >
                    <div className={`relative p-6 lg:p-8 rounded-2xl bg-gradient-to-br ${ad.color} backdrop-blur-sm border border-white/20 shadow-2xl hover:shadow-orange-500/20 transition-all duration-300`}>
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-white font-bold text-lg lg:text-xl">{ad.platform}</h3>
                        ...
                        <div className="text-orange-200 text-2xl lg:text-3xl font-bold mb-2">{ad.stats}</div>
                        <div className="text-white/80 text-sm lg:text-base">Campaign Performance</div>
                      </div>
                    </div>
                  </div>
                ))}
